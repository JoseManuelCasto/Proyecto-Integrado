...
public class ModeloPrenda {
    ...
    public static List<Prenda> obtenerPrendas(int cantidad) {
        EntityManagerFactory emf = Persistence.createEntityManagerFactory("FashionXpressPU");
        PrendaJpaController pjc = new PrendaJpaController(emf);
        List<Prenda> prendas = pjc.findPrendaEntities();
        Collections.shuffle(prendas);
        List<Prenda> prendasAleatorias = new ArrayList();

        for (int i = 0; i < cantidad; i++) {
            prendasAleatorias.add(prendas.get(i));

        }
        emf.close();
        return prendasAleatorias;
...
//Se podra editar proveedor?
    ...
    public static void editarPrenda(String nombre, String[] tallas_disp, String descripcion, double precio, String imagenPrenda, int stock, Prenda p) {
        EntityManagerFactory emf = Persistence.createEntityManagerFactory("FashionXpressPU");
        PrendaJpaController pjc = new PrendaJpaController(emf);
    ...
    public static void anadirPrenda(String nombre, Proveedor proveedor, String[] tallas_disp, String descripcion, double precio, String imagenPrenda, int stock, String marca) {
        EntityManagerFactory emf = Persistence.createEntityManagerFactory("FashionXpressPU");
        PrendaJpaController pjc = new PrendaJpaController(emf);
        List<String> tallas = new ArrayList();
        Prenda p = new Prenda();
